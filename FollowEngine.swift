...
final class FollowEngine {
    ...
    private struct Cfg {
        ...
        var targetWidthRange: ClosedRange<CGFloat> = 0.43...0.50
    ...
    private func stabilize(previous prev: CGRect, target raw: CGRect, dt: CGFloat) -> CGRect {
        ...
        if isDoingExerciseInPlace || zoomFrozen {
            ...
            targetH = softDeadbandScale(prev: prev.height, target: targetH, rel: 0.30)
        }
        // 宽度也做相对死区（小一点更灵敏）
        targetW = softDeadbandScale(prev: prev.width, target: targetW, rel: 0.10)

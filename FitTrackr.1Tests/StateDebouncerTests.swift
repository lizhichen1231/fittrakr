...
final class StateDebouncerTests: XCTestCase {
    ...
    private func makeInput(zoom: CGFloat, center: CGPoint? = .zero, vel: CGFloat = 0,
                           tag: String = "L", chain: Int = 999, tier1: Bool = true) -> LensArbiterInput {
    ...
    func testChainGateBlocksAndAdmits() {
        ...
        for _ in 0..<300 {                       // 链长达标 → 驻留 1.5s 后放行一次
    ...
    func testHysteresisAsymmetry() {
        ...
        // 进 Wide:中心 0、zoom 2.4、静止,驻留 1.5s 后应切
        XCTAssertEqual(run(120, zoom: 2.4, cx: 0), [.toWide], "前置门满足+驻留 1.5s → 恰一次 toWide")
        ...
        // 回门线内(0.10):冷却 5s 内不得重进;冷却过后 + 驻留 1.5s → 重进
    ...
    func testCooldownSingleDirection() {
        ...
        // 进 Wide(toWide 不写冷却)
        var got: [LensCommand] = []
        for _ in 0..<120 { let c = step(zoom: 2.4, cx: 0); if c != .none { got.append(c) } }
